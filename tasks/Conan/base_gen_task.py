...
def get_pitch(wav_data, mel, hparams):
    ...
    lpad = pad_size * 2
    rpad = max(len(mel) - len(f0) - lpad,0)
...
# This task is only used as base_task
class AuxDecoderMIDITask(FastSpeechTask):
    ...
    def test_step(self, sample, batch_idx):
        ...
        self.saving_result_pool.add_job(self.save_result, args=[
            wav_pred, mel_pred, base_fn % 'P', gen_dir, None, None, f0, f0_pred, None])
        if hparams['save_gt']:
            wav_gt = self.vocoder.spec2wav(mel_gt)
            self.saving_result_pool.add_job(self.save_result, args=[
                wav_gt, mel_gt, base_fn % 'G', gen_dir, None,None, f0, f0_pred, None])
