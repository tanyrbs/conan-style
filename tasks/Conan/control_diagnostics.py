...
STYLE_SUCCESS_OUTPUT_DIAG_MAPPING = (
    ...
    ("style_success_proxy_batch_scale", "diag_style_success_proxy_batch_scale"),
    ("style_success_proxy_target_batch", "diag_style_success_proxy_target_batch"),
    ("style_success_label_proxy_sufficient_row_frac", "diag_style_success_label_proxy_sufficient_row_frac"),
    ("style_success_label_authority_frac", "diag_style_success_label_authority_frac"),
    ("style_success_label_authority_row_frac", "diag_style_success_label_authority_row_frac"),
    ("style_success_batch_composition_scale", "diag_style_success_batch_composition_scale"),
    ...
    ("style_success_label_mean_negatives_per_row", "diag_style_success_label_mean_negatives_per_row"),
    (
    ...
    ("style_success_mean_negatives_per_valid_row", "diag_style_success_mean_negatives_per_valid_row"),
    ("style_success_proxy_informative_feature_count", "diag_style_success_proxy_informative_feature_count"),
    ("style_success_proxy_feature_count", "diag_style_success_proxy_feature_count"),
    ("style_success_label_negative_pair_density", "diag_style_success_label_negative_pair_density"),
    ("style_success_proxy_negative_pair_density", "diag_style_success_proxy_negative_pair_density"),
    ("style_success_rank_min_negative_row_frac", "diag_style_success_rank_min_negative_row_frac"),
    (
        "style_success_rank_min_mean_negatives_per_row",
        "diag_style_success_rank_min_mean_negatives_per_row",
    ),
    ("style_success_rank_min_effective_support", "diag_style_success_rank_min_effective_support"),
    ("style_success_proxy_min_informative_features", "diag_style_success_proxy_min_informative_features"),
)

STYLE_SUCCESS_TARGET_SOURCE_FLAGS = (
...
def _record_output_tensor_mapping(diagnostics, source, mapping):
    for source_key, diag_key in mapping:
        value = source.get(source_key)
        if isinstance(value, torch.Tensor):
            diagnostics[diag_key] = value.detach().float().mean()


def _record_style_success_target_source_diagnostics(
    diagnostics,
    *,
    style_success_target_source,
    style_success_memory_fallback_used,
):
    ...
    for target_source in STYLE_SUCCESS_TARGET_SOURCE_FLAGS:
        diagnostics[f"diag_style_success_target_source_{target_source}"] = torch.tensor(
            1.0 if style_success_target_source == target_source else 0.0
        )
...
def _record_style_success_row_fraction_diagnostics(
    diagnostics,
    *,
    label_valid_rows,
    proxy_valid_rows,
    ...
    if isinstance(valid_rows, torch.Tensor):
        valid_row_frac = valid_rows.float().mean()
        diagnostics["diag_style_success_negative_row_frac"] = valid_row_frac
        diagnostics["diag_style_success_rank_valid_row_frac"] = valid_row_frac


def _record_style_success_negative_state_scalars(diagnostics, negative_state, *, device):
    for state_key, diag_key in (
        ...
        ("mean_negatives_per_valid_row", "diag_style_success_mean_negatives_per_valid_row"),
        ("label_negative_pair_density", "diag_style_success_label_negative_pair_density"),
        ("proxy_negative_pair_density", "diag_style_success_proxy_negative_pair_density"),
        ("proxy_backfill_row_frac", "diag_style_success_proxy_backfill_row_frac"),
    ):
        value = negative_state.get(state_key)
        if isinstance(value, torch.Tensor):
            diagnostics[diag_key] = value.detach().float().mean()
        elif isinstance(value, (int, float)):
            diagnostics[diag_key] = torch.tensor(float(value), device=device)
...
def _record_style_success_support_state_scalars(diagnostics, support_state):
    _record_output_tensor_mapping(
        diagnostics,
        support_state,
        (
            ("effective_support", "diag_style_success_rank_effective_support"),
            ("gate_passed", "diag_style_success_rank_gate_passed"),
            ("rank_term_active", "diag_style_success_rank_term_active"),
            ("proxy_batch_scale", "diag_style_success_proxy_batch_scale"),
            ("proxy_target_batch", "diag_style_success_proxy_target_batch"),
            (
                "label_proxy_sufficient_row_frac",
                "diag_style_success_label_proxy_sufficient_row_frac",
            ),
            ("label_authority_frac", "diag_style_success_label_authority_frac"),
            (
                "min_label_authority_row_frac",
                "diag_style_success_label_authority_row_frac",
            ),
            ("batch_composition_scale", "diag_style_success_batch_composition_scale"),
        ),
    )


def _record_style_success_negative_source_diagnostics(diagnostics, negative_source, *, device):
    for source_name in ("label", "proxy", "label_plus_proxy_backfill"):
        diagnostics[f"diag_style_success_negative_source_{source_name}"] = torch.tensor(
            1.0 if negative_source == source_name else 0.0,
            device=device,
        )


def _record_style_success_disabled_reason_diagnostics(diagnostics, support_state, *, device):
    disabled_reason = str(support_state.get("disabled_reason", "unknown"))
    disabled_reason_map = {
        "active": "diag_style_success_rank_disabled_reason_active",
        "no_valid_negative_rows": "diag_style_success_rank_disabled_reason_no_valid_rows",
        "negative_row_density_below_floor": "diag_style_success_rank_disabled_reason_low_row_density",
        "mean_negatives_per_row_below_floor": "diag_style_success_rank_disabled_reason_low_negatives",
        "proxy_informative_features_below_floor": "diag_style_success_rank_disabled_reason_low_proxy_features",
        "proxy_batch_support_below_floor": "diag_style_success_rank_disabled_reason_proxy_batch_support_low",
        "proxy_backfill_support_below_floor": "diag_style_success_rank_disabled_reason_proxy_backfill_support_low",
        "effective_support_below_floor": "diag_style_success_rank_disabled_reason_low_effective_support",
    }
    for reason, diag_key in disabled_reason_map.items():
        diagnostics[diag_key] = torch.tensor(
            1.0 if disabled_reason == reason else 0.0,
            device=device,
        )


def _record_style_success_hard_negative_diagnostics(
    diagnostics,
    *,
    anchor_bank,
    target_bank,
    ...
    if not (isinstance(valid_rows, torch.Tensor) and valid_rows.any()):
        return
    similarity = anchor_bank @ target_bank.transpose(0, 1)
    hard_negative = similarity.masked_fill(~negative_mask, float("-inf")).max(dim=-1).values
    hard_negative = hard_negative[valid_rows]
    if hard_negative.numel() <= 0 or not torch.isfinite(hard_negative).any():
        return
    ...
    diagnostics["diag_style_success_pair_margin"] = pair_cos[valid_rows].mean() - hard_negative.mean()
...
def _record_style_success_diagnostics(
    ...
    *,
    ...
    _record_style_success_target_source_diagnostics(
        diagnostics,
        style_success_target_source=style_success_target_source,
        style_success_memory_fallback_used=style_success_memory_fallback_used,
    )
    ...
    _record_style_success_row_fraction_diagnostics(
        diagnostics,
        label_valid_rows=label_valid_rows,
        proxy_valid_rows=proxy_valid_rows,
        valid_rows=valid_rows,
    )
    _record_style_success_negative_state_scalars(
        diagnostics,
        negative_state,
        device=anchor_bank.device,
    )
    _record_style_success_support_state_scalars(diagnostics, support_state)
    _record_style_success_negative_source_diagnostics(
        diagnostics,
        negative_source,
        device=anchor_bank.device,
    )
    _record_style_success_disabled_reason_diagnostics(
        diagnostics,
        support_state,
        device=anchor_bank.device,
    )
    _record_style_success_hard_negative_diagnostics(
        diagnostics,
...
class ControlDiagnosticsState:
    style_trace: object
    style_trace_mask: object
    slow_style_trace: object
    dynamic_timbre: object
    style_repr: object
    fast_style_repr: object
    slow_style_repr: object
    timbre_repr: object
    global_timbre_anchor: object
    global_style_summary: object
    output_identity_embed: object
    reference_identity_embed: object
    style_query_repr: object
    timbre_query_repr: object
    tensor_kwargs: dict



def _build_control_diagnostics_state(output):
    ...
    for value in (
        ...
        if isinstance(value, torch.Tensor):
            tensor_kwargs["device"] = value.device
            break
    return ControlDiagnosticsState(
        style_trace=style_trace,
        style_trace_mask=style_trace_mask,
        slow_style_trace=slow_style_trace,
        dynamic_timbre=dynamic_timbre,
        style_repr=style_repr,
        fast_style_repr=fast_style_repr,
        slow_style_repr=slow_style_repr,
        timbre_repr=timbre_repr,
        global_timbre_anchor=global_timbre_anchor,
        global_style_summary=global_style_summary,
        output_identity_embed=output_identity_embed,
        reference_identity_embed=reference_identity_embed,
        style_query_repr=style_query_repr,
        timbre_query_repr=timbre_query_repr,
        tensor_kwargs=tensor_kwargs,
    )



def _record_style_strength_diagnostics(diagnostics, sample):
...
def _record_representation_diagnostics(diagnostics, output, sample, config, state):
    style_valid = _mask_valid_fraction(state.style_trace_mask, state.style_trace)
    if style_valid is not None:
        diagnostics["diag_style_trace_valid_frac"] = style_valid
    slow_style_valid = _mask_valid_fraction(output.get("slow_style_trace_mask"), state.slow_style_trace)
    if slow_style_valid is not None:
        diagnostics["diag_slow_style_trace_valid_frac"] = slow_style_valid
    timbre_valid = _mask_valid_fraction(output.get("dynamic_timbre_mask"), state.dynamic_timbre)
    if timbre_valid is not None:
        diagnostics["diag_dynamic_timbre_valid_frac"] = timbre_valid

    style_trace_norm, _ = _safe_mean_std(
        state.style_repr.norm(dim=-1) if isinstance(state.style_repr, torch.Tensor) else None
    )
    if style_trace_norm is not None:
        diagnostics["diag_style_trace_norm"] = style_trace_norm
    timbre_norm, _ = _safe_mean_std(
        state.timbre_repr.norm(dim=-1) if isinstance(state.timbre_repr, torch.Tensor) else None
    )
    ...
    style_query_norm, style_query_norm_std = _safe_mean_std(
        state.style_query_repr.norm(dim=-1) if isinstance(state.style_query_repr, torch.Tensor) else None
    )
    ...
    timbre_query_norm, timbre_query_norm_std = _safe_mean_std(
        state.timbre_query_repr.norm(dim=-1) if isinstance(state.timbre_query_repr, torch.Tensor) else None
    )
    ...
    style_timbre_cos = _cosine_mean(state.style_repr, state.timbre_repr)
    ...
    style_global_cos = _cosine_mean(state.style_repr, state.global_style_summary)
    if style_global_cos is not None:
        diagnostics["diag_style_global_cos"] = style_global_cos
    slow_style_global_cos = _cosine_mean(state.slow_style_repr, state.global_style_summary)
    ...
    _record_style_success_diagnostics(
        ...
        sample,
        config,
        style_repr=state.style_repr,
        slow_style_repr=state.slow_style_repr,
        fast_style_repr=state.fast_style_repr,
    )
    timbre_anchor_cos = _cosine_mean(state.timbre_repr, state.global_timbre_anchor)
    if timbre_anchor_cos is not None:
        diagnostics["diag_timbre_anchor_cos"] = timbre_anchor_cos
    query_cos = _cosine_mean(
        state.style_query_repr,
        state.timbre_query_repr,
    )
...
def _resolve_output_identity_target_embed(output):
    ...
    return output_identity_target_embed



def _record_identity_diagnostics(diagnostics, output, state):
    output_identity_target_embed = _resolve_output_identity_target_embed(output)
    ...
    if target_source is not None:
        diagnostics["diag_output_identity_target_source_is_reference"] = _categorical_indicator(
            target_source,
            "reference_identity_embed",
            device=state.tensor_kwargs.get("device", None),
        )
    ...
    output_identity_cos = _cosine_mean(state.output_identity_embed, state.global_timbre_anchor)
    ...
    output_identity_ref_cos = _cosine_mean(state.output_identity_embed, state.reference_identity_embed)
    ...
    output_identity_norm, output_identity_norm_std = _safe_mean_std(
        state.output_identity_embed.norm(dim=-1) if isinstance(state.output_identity_embed, torch.Tensor) else None
    )
...
def _record_runtime_budget_diagnostics(diagnostics, output, sample, config):
    ...
    return budget_support_weight



def _record_runtime_flag_diagnostics(diagnostics, output, config, state):
    tensor_kwargs = state.tensor_kwargs
...
def _record_stage_budget_diagnostics(diagnostics, output, config, budget_support_weight):
    budget_ratio = float(
        output.get(
...
def _record_decoder_router_and_context_diagnostics(diagnostics, output, config, state):
    diagnostics.update(
...
def _record_tracked_lambda_diagnostics(diagnostics, config):
    if not config.get("control_diagnostics_track_lambdas", True):
        return
    device = None
...
def collect_control_diagnostics(output, sample, config):
    ...
    diagnostics = {}
    with torch.no_grad():
        state = _build_control_diagnostics_state(output)
        _record_style_strength_diagnostics(diagnostics, sample)
        _record_representation_diagnostics(diagnostics, output, sample, config, state)
        _record_identity_diagnostics(diagnostics, output, state)
        budget_support_weight = _record_runtime_budget_diagnostics(diagnostics, output, sample, config)
        _record_runtime_flag_diagnostics(diagnostics, output, config, state)
        _record_stage_budget_diagnostics(diagnostics, output, config, budget_support_weight)
        _record_decoder_router_and_context_diagnostics(diagnostics, output, config, state)
        _record_tracked_lambda_diagnostics(diagnostics, config)
    return diagnostics
