...
# Different length sequences need masks
def sum_except_batch_with_padding(x, num_dims=1,nonpadding_mask=None):
    # x [b, t]
    # padding_mask [b, t]
    x = x * nonpadding_mask
...
# ctt_T = 0.1 => uniform only, ctt_T=0.99999 => mask only
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        self.amp = False
        self.transformer = denoise_fn
        self.num_classes = hparams["f0_num_classes"] # 256
    ...
    def predict_start(self, log_x_t, cond_emb, t, x_mask):          # p(x0|xt)
        ...
        zero_vector = torch.zeros(batch_size, 1, content_seq_len).type_as(log_x_t)- 70
        log_pred = torch.cat((log_pred, zero_vector), dim=1)
        log_pred = torch.clamp(log_pred, -70, 0)

        return log_pred
    
    def cf_predict_start(self, log_x_t, cond_emb, t, x_mask):
        return self.predict_start(log_x_t, cond_emb, t, x_mask)
    ...
    def q_posterior(self, log_x_start, log_x_t, t):            # p_theta(xt_1|xt) = sum(q(xt-1|xt,x0')*p(x0'))
        ...
        content_seq_len = onehot_x_t.shape[1]
    ...
    def p_pred(self, log_x, cond_emb, t, x_mask):             # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
        if self.parametrization == 'x0':
            log_x_recon = self.cf_predict_start(log_x, cond_emb, t, x_mask)
            log_model_pred = self.q_posterior(
                log_x_start=log_x_recon, log_x_t=log_x, t=t)
        elif self.parametrization == 'direct':
            log_model_pred = self.predict_start(log_x, cond_emb, t, x_mask)
        else:
            raise ValueError
        return log_model_pred, log_x_recon

    @torch.no_grad()
    def p_sample(self, log_x, cond_emb, t, x_mask, sampled=None, to_sample=None):               # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
    ...
    def _train_loss(self, x, cond_emb, nonpadding,is_train=True):                       # get the KL loss
        # nonpadding [b, t]
        ...
        log_x0_recon = self.predict_start(log_xt, cond_emb, t=t, x_mask=nonpadding)            # P_theta(x0|xt)
        ...
        decoder_nll = sum_except_batch_with_padding(decoder_nll, nonpadding_mask=nonpadding)
        ...
        if self.auxiliary_loss_weight != 0 and is_train==True:
            ...
            kl_aux = sum_except_batch_with_padding(kl_aux, nonpadding_mask=nonpadding)
    ...
    def forward(
        ...
        if kwargs.get('autocast') == True:
            self.amp = True
        log_model_prob, loss = self._train_loss(content_token, cond, nonpadding=nonpadding)
        loss = loss.sum()/(content_token.size()[0]) # Note: in _train_loss stage, loss has already been averaged over time dimension

        # 4) get output, especially loss
        ret['logits'] = torch.exp(log_model_prob)
        ret['fdiff'] = loss 
        self.amp = False
        return ret
    ...
    def sample(
        ...
        zero_logits = torch.zeros((batch_size, self.num_classes-1, cond.shape[1]),device=device)
        one_logits = torch.ones((batch_size, 1, cond.shape[1]),device=device)
        ...
        with torch.no_grad():
            for diffusion_index in range(start_step-1, -1, -1):
                ...
                while min(sampled) < self.n_sample[diffusion_index]:
                    log_z, sampled = self.p_sample(log_z, cond, t, nonpadding, sampled, self.n_sample[diffusion_index])     # log_z is log_onehot
        
        content_token = log_onehot_to_index(log_z)
        ret["content_token"] = content_token
        ret["logits"] = torch.exp(log_z)
        return ret
