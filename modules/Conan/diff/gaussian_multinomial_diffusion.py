...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def mixed_loss(self, f0, uv, cond, nonpadding, out_dict):
        ...
        return loss_multi.mean(), loss_gauss.mean(), nll_loss.mean()
...
class GaussianMultinomialDiffusionx0(GaussianMultinomialDiffusion):
    def gaussian_p_mean_variance(self, x, t, noise_pred, clip_denoised=False, dyn_clip=None):
        x_recon = noise_pred
    ...
    def mixed_loss(self, f0, uv, cond, nonpadding, out_dict):
        ...
        loss_gauss = self.gaussian_p_losses(model_out_num, x_num, nonpadding=nonpadding * (uv == 0).float())
