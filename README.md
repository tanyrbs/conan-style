...
- `mainline_train_prep.py` now supports hard-acceptance binary audits via `--full_binary_scan` (or `--binary_scan_items <= 0`) and records `binary_frame_alignment_scan_mode` / `binary_frame_alignment_scan_limit` in the JSON summary instead of silently looking like an exhaustive check when it was only sampled
- `ConanDataset` still trims misaligned framewise features defensively to keep training from exploding, but it now also exposes trim diagnostics and can emit capped warnings via `dataset_warn_on_frame_trim: true` so stale sidecars are easier to notice instead of being silently tolerated forever
...
- **Inference config singleton pollution:** `inference/Conan.py` still exposes `legacy_global_hparams_bridge`, but built-in `HifiGAN` / `HifiGAN_NSF` inference now defaults that bridge to **disabled** and the shipped wrappers consume local runtime hparams instead of relying on process-global state. Custom vocoders still need an explicit local-hparams-safe constructor contract (or an explicit bridge opt-in), so concurrent multi-instance library use is only isolation-safe once downstream dependencies actually honor that local contract.
...
Canonical mainline now also keeps `global_style_summary_fallback_to_timbre: false` as the fail-closed default, so missing style-summary evidence no longer silently aliases the global timbre anchor unless you opt that fallback back in on purpose.
