...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not auto-generate factorized swap report even if swap_variant rows are present.",
    )
...
def main():
    ...
    if not args.skip_factorized_report and any(row.get("swap_variant") is not None for row in rows):
