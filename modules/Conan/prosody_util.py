...
def _make_guided_attention_mask(ilen, rilen, olen, rolen, sigma):
    grid_x, grid_y = torch.meshgrid(
        torch.arange(ilen, device=rilen.device),
        torch.arange(olen, device=rolen.device),
        indexing="ij",
    )
...
class LocalTimbreAdaptor(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.padding_idx = padding_idx
        self.hidden_size = hidden_size
        self.use_vq = use_vq and num_vq_codes > 0
        self.n_embed = num_vq_codes
        if self.use_vq:
            self.vqvae = VQEmbeddingEMA(
                self.n_embed,
    ...
    def forward(self, ref_mels, mel2ph=None):
        ...
        if mel2ph is not None:
            ref_units, _ = group_hidden_by_segs(ref_mels, mel2ph, torch.max(mel2ph))
        else:
            ref_units = ref_mels
        timbre = self.encoder(ref_units)
        if not self.use_vq:
            zero = timbre.new_zeros(())
            return timbre, zero, zero
        z, vq_loss, _, ppl = self.vqvae(timbre)
        vq_loss = vq_loss.mean()
        return z, vq_loss, ppl
