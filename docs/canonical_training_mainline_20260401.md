...
Canonical mainline also keeps `global_style_summary_fallback_to_timbre: false` as the fail-closed default, so missing style-summary evidence now raises instead of silently collapsing back onto the timbre anchor unless you explicitly opt that fallback back in.
...
- **Inference/runtime singleton bridge:** the inference engine still exposes `legacy_global_hparams_bridge`, but built-in `HifiGAN` / `HifiGAN_NSF` inference now defaults that bridge to disabled and passes local runtime hparams into the shipped vocoder wrappers. Custom vocoders still need an explicit local-hparams-safe constructor contract (or an explicit bridge opt-in), so concurrent multi-instance library use is only isolation-safe once downstream modules actually honor that local contract.
...
For pre-train data acceptance, `mainline_train_prep.py` now supports both the fast sampled binary audit and a true exhaustive mode:

- default: sampled frame-alignment audit (`--binary_scan_items`, default `64`)
- hard acceptance: `--full_binary_scan` or `--binary_scan_items <= 0`

The emitted JSON now records `binary_frame_alignment_scan_mode` plus `binary_frame_alignment_scan_limit`, so later reviews can tell whether a prep result came from a sample or a real full scan.

At runtime, `ConanDataset` still keeps the defensive "trim to common shortest frame length" guard so training does not crash on stale sidecars, but it also records trim diagnostics and can emit capped warnings (`dataset_warn_on_frame_trim`, `dataset_warn_on_frame_trim_max_events`) so that guard does not become a totally silent data-quality sink.
